...
function cache() {
    # We cache requests for 24 hours so anilist dont hate us and implement rate limits in the future.
    echo "[*  ] Checking cache"
    mkdir -p "$CACHE/$MANGA_ID"
    ...
    if [ $(("$CURRENT_TIME"-"$LAST_MODIFIED")) -lt "86400" ]  # Rate limit checker
    then
...
function create_template() {
    ...
    PENCILLER=$WRITER  # For most series I've tested they're the same person so I will just assume that to be the case. If you find any series that does not follow this please let me know so I can do this better
